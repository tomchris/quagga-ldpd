/*
 * Copyright (C) 2015 by NetDEF, Inc.
 *
...
{
	struct QZCError err_base = {
		.subject_which = QZCError_subject_protocol,
		.codeloc.line = line,
		.category = category,
		.uniqueId = unique_id,
	};
	err_base.codeloc.file.str = file;
	err_base.codeloc.file.len = file ? strlen(file) : 0;
	err_base.codeloc.func.str = func;
	err_base.codeloc.func.len = func ? strlen(func) : 0;
	err_base.message.str = message;
        err_base.message.len = message ? strlen(message) : 0;

        zlog_warn ("QZC message error: %s", message ? message : "<?>");

	assert(rep->error_which == QZCReply_error_noerror);
	rep->error_which = QZCReply_error_error;
	rep->error.error = new_QZCError(cs);
	write_QZCError(&err_base, rep->error.error);
}

void qzc_err_node(struct QZCReply *rep, struct capn_segment *cs, uint64_t nid)
{
	struct QZCError cur_err;
	read_QZCError(&cur_err, rep->error.error);
	assert(cur_err.subject_which == QZCError_subject_protocol);
	cur_err.subject_which = QZCError_subject_node;
	cur_err.subject.node.nid = nid;
	write_QZCError(&cur_err, rep->error.error);
}
		...
		uint64_t nid, uint64_t elem)
{
	...
	read_QZCError(&cur_err, rep->error.error);
	assert(cur_err.subject_which == QZCError_subject_protocol);
	cur_err.subject_which = QZCError_subject_elem;
	cur_err.subject.elem.nid = nid;
	cur_err.subject.elem.elem = elem;
	write_QZCError(&cur_err, rep->error.error);
}
                ...
                const uint32_t *ordinals, size_t len)
{
	...
	read_QZCError(&cur_err, rep->error.error);
	assert(cur_err.subject_which == QZCError_subject_protocol);
	cur_err.subject_which = QZCError_subject_data;
	cur_err.subject.data.nid = nid;
	cur_err.subject.data.elem = elem;
	cur_err.subject.data.ordinals = capn_new_list32(cs, len);
	capn_setv32(cur_err.subject.data.ordinals, 0, ordinals, len);
	write_QZCError(&cur_err, rep->error.error);
}
                            ...
                            struct capn_segment *cs)
{
  ...
  if (!wkn || !wknrep.nid) {
    qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "WKN not found");
    qzc_err_node(rep, cs, wknreq.wid);
  }
                             ...
                             struct capn_segment *cs)
{
  ...
  if (!node) {
    qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "node not found");
    qzc_err_node(rep, cs, nireq.nid);
  }
  nirep.nid = nireq.nid;
                     ...
                     struct capn_segment *cs)
{
  ...
  if (!node || !node->type) {
    qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "parent node not found");
    qzc_err_node(rep, cs, creq.parentnid);
    goto out;
  }
  if (!node->type->createchild) {
    qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "parent node cannot create children");
    qzc_err_node(rep, cs, creq.parentnid);
    goto out;
  }

  void *entity = ((char *)node) - node->type->node_member_offset;
  node->type->createchild(entity, &creq, &crep, rep, cs);
                     ...
                     struct capn_segment *cs, bool unset)
{
  ...
  if (!node || !node->type)
    {
      qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "node not found");
      qzc_err_node(rep, cs, sreq.nid);
      return;
    }
  if (!(unset ? node->type->unset : node->type->set))
    {
      qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "node does not implement write access");
      qzc_err_node(rep, cs, sreq.nid);
      return;
    }
  ...
  void *entity = ((char *)node) - node->type->node_member_offset;
  if (unset)
    node->type->unset(entity, &sreq, rep, cs);
  else
    node->type->set(entity, &sreq, rep, cs);
}
                     ...
                     struct capn_segment *cs)
{
  ...
  if (!node || !node->type)
    {
      qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "node not found");
      qzc_err_node(rep, cs, dreq.nid);
      return;
    }
  if (!node->type->destroy)
    {
      qzc_err_set(rep, cs, QERR_CONFIG_INVALID, 0, "node not destroyable");
      qzc_err_node(rep, cs, dreq.nid);
    }

  void *entity = ((char *)node) - node->type->node_member_offset;
  node->type->destroy(entity, &dreq, rep, cs);
}
...
static void qzc_callback (void *arg, void *zmqsock, zmq_msg_t *msg)
{
  ...
  rep.error_which = QZCReply_error_noerror;
  ...
  zlog_info ("QZC request type %d, response type %d, %zd bytes", req.which, rep.which, rs);
